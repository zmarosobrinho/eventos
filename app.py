...
def index():
    ...
    city_counts = (
        db.session.query(Event.city, db.func.count(Event.id))
        .group_by(Event.city)
        .all()
    )
    city_data = [{"city": city, "count": count} for city, count in city_counts]
    return render_template('index.html', events=events, city_data=city_data, user=current_user())
